...
const Mapping: React.FC = () => {
  ...
  const { selectedSubDistricts, display_raster, setdisplay_raster } = useLocation();
  ...
  useEffect(() => {
    ...
    console.log("layer filer", LayerFilter);
    console.log("filter vlau e", LayerFilterValue);
      ...
      if (zoom > 5 && featureName) {
        const labelStyle = new Style({
          text: new Text({
            ...
            overflow: true,
            maxAngle: Math.PI / 4,
            placement: geometryType === "Point" ? "point" : "polygon",
            scale: zoom > 15 ? 1.2 : 1,
          }),
        });
      ...
      >
        ...
        {(loading || isMapLoading || stpOperation) && (
          <div className="absolute inset-0 flex items-center justify-center bg-opacity-25 backdrop-blur-sm z-50 transition-all duration-500 animate-fade-in">
            <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl p-6 max-w-sm flex items-center space-x-4 transition-all transform animate-float border border-gray-200">
